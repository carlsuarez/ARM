 ...
 */
static uint8_t search_directory(uint32_t cluster, char *name, fat32_dir_entry_t *result);

/**
 * @brief Reads the contents of a file starting from a specific cluster.
 *
 * @param cluster The starting cluster of the file.
 * @param buf The buffer to store the file contents.
 * @param size The number of bytes to read.
 * @return int8_t Returns 0 on success, or a negative value on failure.
 */
static int8_t fat32_read_file_contents(uint32_t cluster, uint8_t *buf, uint32_t size);
...
static int8_t fat32_read_file_contents(uint32_t cluster, uint8_t *buf, uint32_t size)
{
    printk("Reading file contents...\n");
    uint8_t sector[SECTOR_SIZE];

    while (!fat32_is_eoc(cluster) && size > 0)
    {
        uint32_t lba = cluster_to_lba(cluster);
        for (uint32_t s = 0; s < fat32_info.sectors_per_cluster && size > 0; ++s)
        {
            ...
            uint32_t num_reading = (size > SECTOR_SIZE) ? SECTOR_SIZE : size;
            memcpy(buf, sector, num_reading);
            buf += num_reading;
            size -= num_reading;
        }
        cluster = fat32_traverse(cluster);
...
int8_t fat32_read_file(const char *path, uint8_t *buf, uint32_t size)
{
    ...
    while (1)
    {
        ...
        if (!search_directory(current_cluster, name, &current_dir))
        {
        ...
        {
            if (current_dir.attr == DIRECTORY)
            {
                printk("File '%s' specified is a directory\n", path);
                return -1;
            }
            else
            {
    ...
    return fat32_read_file_contents(current_cluster, buf, size);
}
